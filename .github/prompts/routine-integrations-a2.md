# Routine A2 — integrations-squad weekly synthesis reply processing (Pass 2)
...
- **Git / repo:** clone fresh at `/tmp/dev-tools-integrations-marketplace-kb` if not already present:
  ...
  Fetch + check out the pushed `weekly-sync/integrations-synthesis-` branch (no PR exists yet). Push
  with
...
- **GitHub (find the branch):** there is no PR to list yet. Use
  `git ls-remote --heads https://toniaquino:$GH_PAT@github.com/toniaquino/dev-tools-integrations-marketplace-kb.git 'weekly-sync/integrations-synthesis-*'`
...
matching `weekly-sync/integrations-synthesis-*` exists, DM `U01EHRXEBFE` a one-line "no pending sync
...
1. List remote branches matching `weekly-sync/integrations-synthesis-*` via `git ls-remote --heads`.
...
If no `weekly-sync/integrations-synthesis-` branch can be found, DM `U01EHRXEBFE`:
...
in Step 2 below, no need to call it twice). If any reply from `U01EHRXEBFE`, trimmed and
...
3. Check out the `weekly-sync/integrations-synthesis-` branch in `/tmp/dev-tools-integrations-marketplace-kb`
...
4. Apply the correction to the relevant `Status.md` or `feature-index.yaml` file
   (both under `02 Teams/Integrations/product-development/`).
...
5. Commit and push to the same branch:
   `incorporate thread correction from Todd Willms [YYYY-MM-DD]`.
...
2. If a correction was folded in during Step 2, append to the body:
   `\n\n---\n_Correction from Todd Willms applied from the Slack thread before this PR was opened._`
...
DM `U01EHRXEBFE` directly:

```
integrations-squad weekly synthesis Pass 2 complete — [YYYY-MM-DD]
...
- Pass 2 summary DMed to `U01EHRXEBFE`.

## Failure handling

- No `weekly-sync/integrations-synthesis-` branch found: DM `U01EHRXEBFE` (see Gate) and exit
  cleanly.
...
- A reply exists but could not be parsed into a concrete change: do not
  ...
  asking for clarification (see Step 2), and DM `U01EHRXEBFE` noting the
  branch is still pending and will be checked again next cycle (or on manual trigger).
- PR-creation API call fails after confirmation: DM `U01EHRXEBFE` with the branch name,
  confirmation status, and the error — the confirmed content is safe on the branch even
  though the PR didn't open.
- Push fails: log the error, DM `U01EHRXEBFE` with the branch name and error.
- Any uncaught fatal error: DM `U01EHRXEBFE` with the routine name, the failed step, and
