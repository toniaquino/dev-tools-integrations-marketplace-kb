# Routine A: integrations-squad weekly synthesis

You are running the weekly synthesis routine for the **integrations-squad** Team OS repo as a
...
(`#api-team`) under any DRY_RUN value.** That channel (and any engineering channel) is
scanned for data only — never written to. All notifications go to `#distr-team-kb-log` and
the PM DM, per the Slack notification format below.
...
- **`DRY_RUN=true` (default):** Do the full scan, analysis, and file generation. Create
  ...
  PM** (Todd Willms, `U01EHRXEBFE`), both prefixed with `[DRY RUN]`.
- **`DRY_RUN=false`:** Communicate normally — post to `#distr-team-kb-log` and DM the PM per
  the steps below. No `[DRY RUN]` prefix. Still never post to `#api-team`.
...
```
SQUAD_NAME:        integrations-squad
PM_NAME:           Todd Willms
PM_SLACK_ID:       U01EHRXEBFE
...
MAIN_BRANCH:       main
SQUAD_CHANNEL:     #api-team
SQUAD_CHANNEL_ID:  G013Z0YLS15
JIRA_BOARD_KEY:    API
...
Slack thread confirms (see routine-integrations-a2.md). Routine A2 does not attempt a
reviewer-request call at all: `GH_PAT` authenticates as `toniaquino`, the same identity
that opens the PR, and GitHub rejects self-review requests outright. The Phase 3 DM to
Todd already carries the PR URL and is the actual notification.
...
# 2. Read 02 Teams/Integrations/CLAUDE.md doc indexes to navigate -- do not explore blindly.
...
git -C /tmp/dev-tools-integrations-marketplace-kb checkout -b weekly-sync/integrations-synthesis-[YYYY-MM-DD]
...
COMMIT_MSG
git -C /tmp/dev-tools-integrations-marketplace-kb push origin weekly-sync/integrations-synthesis-[YYYY-MM-DD]
# 5. Do NOT open a PR here. Routine A2 (Pass 2) opens the PR once the Slack thread gets
#    a 👍 or a parseable correction reply -- see routine-integrations-a2.md. This routine's
#    job ends once the branch is pushed.
# 6. Notify per the DRY_RUN rules above, linking the compare view since there is no PR
#    yet: https://github.com/toniaquino/dev-tools-integrations-marketplace-kb/compare/main...weekly-sync/integrations-synthesis-[YYYY-MM-DD]
...
curl -s -u "$ATLASSIAN_EMAIL:$ATLASSIAN_API_TOKEN" \
  ...
  --data-urlencode "jql=project = API AND updated >= -7d ORDER BY updated DESC" \
...
`conversations.history` requires a channel ID, not a channel name. **Use the hardcoded ID
from Global context (`SQUAD_CHANNEL_ID`) directly -- do not re-resolve the channel name
via `conversations.list` on every run.** This ID was confirmed live against the workspace
on 2026-07-29 (`#api-team` = `G013Z0YLS15`).

Fetch the last 7 days of messages, via Bash:
```
curl -s -H "Authorization: Bearer $SLACK_BOT_TOKEN" \
  -G "https://slack.com/api/conversations.history" \
  --data-urlencode "channel=G013Z0YLS15" \
...
Read `02 Teams/Integrations/CLAUDE.md` and its doc indexes only -- **not** the repo root
`CLAUDE.md`, which is the shared Dev Tools/Integrations/Marketplace overview covering
all three squads in this repo, not Integrations specifically. Do **not** read `Status.md`
or PRD files directly yet. For each initiative in that CLAUDE.md's initiative list:

- **Slack:** scan the squad channel (`#api-team`) and the engineering channel for
  the last 7 days (see "Slack API access" above for how to query this). Extract status
  mentions, decision language, blocker language, and Jira ticket references. Use the
  PM-confirmed (✅) channels listed in that CLAUDE.md's table.
- **Jira:** query all active tickets on board `API` (see "Jira API access" above for how
  to query this). Extract status changes in the last 7 days.
- **Apply source priority:** squad area `Status.md` (current, as baseline) > Jira > Slack.

### Phase 2: update files

For each functional area, read its area `CLAUDE.md` and current `Status.md` files (all
under `02 Teams/Integrations/product-development/`), then:
...
```
project = API AND issuetype = Epic
...
`02 Teams/Integrations/product-development/product/feature-index.yaml`. For each shipped epic from Step 1, find
...
synthesis ask -- do not send a separate DM or a separate #api-team post for
...
### Phase 3a: schedule the Slack notification for Todd Willms's local 9am

**TEMPORARY (2026-07-20): hardcoded timezone.** Todd Willms's Slack timezone is
**America/Los_Angeles** (US Pacific). A live `users.info` lookup would be more robust
...
2. Compute today's 9:00 AM in `America/Los_Angeles` as a UTC Unix timestamp
   (self-corrects for DST — no manual offset math):
   `target_utc=$(TZ="America/Los_Angeles" date -d "09:00" +%s)`.
3. If `target_utc` is already in the past (≤ now — e.g. Todd's 9am already passed
   before this run started), recompute for tomorrow instead:
   `target_utc=$(TZ="America/Los_Angeles" date -d "09:00 tomorrow" +%s)`.
...
### Sources used
[Jira tickets (API-XXXX), Slack channels referenced]
...
Post to `#distr-team-kb-log` (<#C0BL0BZQFNX>) and DM the PM. Never post to `#api-team` — it
is scanned for data only, never written to, regardless of DRY_RUN.

There is no PR yet at this point -- link the branch's compare view instead:
`https://github.com/toniaquino/dev-tools-integrations-marketplace-kb/compare/main...weekly-sync/integrations-synthesis-[YYYY-MM-DD]`

When `DRY_RUN=false`:

```
<@U01EHRXEBFE> -- integrations-squad weekly synthesis proposed, not yet a PR.
...
```
[DRY RUN] <@U01EHRXEBFE> -- integrations-squad weekly synthesis (preview).
What changed: [1-2 line summary]
...
- **No changes detected:** do not push a branch (there is nothing for Routine A2 to open
  ...
  week.` (prefixed `[DRY RUN]` when `DRY_RUN=true`). Never post this to `#api-team`.
- **Push fails:** DM the PM (`U01EHRXEBFE`) with the generated Status.md content and the
