# Routine A2 — Technology Partners squad weekly synthesis reply processing (Pass 2)
...
- **Git / repo:** clone fresh at `/tmp/dev-tools-integrations-marketplace-kb` if not already present:
  ...
  Fetch + check out the pushed `weekly-sync/techpartners-synthesis-` branch (no PR exists yet). Push
  with
...
- **GitHub (find the branch):** there is no PR to list yet. Use
  `git ls-remote --heads https://toniaquino:$GH_PAT@github.com/toniaquino/dev-tools-integrations-marketplace-kb.git 'weekly-sync/techpartners-synthesis-*'`
...
- **GitHub (open the PR, once confirmed):**
  ...
  can be fixed by reconfiguring the token). The Phase 3 DM to Toni, which includes
...
matching `weekly-sync/techpartners-synthesis-*` exists, DM TONI_SLACK_ID a one-line "no pending
Technology Partners sync branch to process this week" and exit cleanly.
...
1. List remote branches matching `weekly-sync/techpartners-synthesis-*` via
...
If no `weekly-sync/techpartners-synthesis-` branch can be found, DM TONI_SLACK_ID:
"No Routine A sync branch found for Technology Partners this week. Pass 1 may not have
...
in Step 2 below, no need to call it twice). If any reply from `U04H9D3H14K`, trimmed and
...
3. Check out the `weekly-sync/techpartners-synthesis-` branch in `/tmp/dev-tools-integrations-marketplace-kb`
   (clone the repo fresh if that path isn't already present).
4. Apply the correction to the relevant `Status.md` or `feature-index.yaml` file
   (both under `02 Teams/Technology Partners/product-development/`).
...
5. Commit and push to the same branch:
   `incorporate thread correction from Bas van Reeuwijk [YYYY-MM-DD]`.
...
2. If a correction was folded in during Step 2, append to the body:
   `\n\n---\n_Correction from Bas van Reeuwijk applied from the Slack thread before this PR was opened._`
...
DM TONI_SLACK_ID directly:

```
Technology Partners weekly synthesis Pass 2 complete — [YYYY-MM-DD]
...
- Pass 2 summary DMed to TONI_SLACK_ID.
...
- A reply exists but could not be parsed into a concrete change: do not
  ...
  asking for clarification (see Step 2), and DM TONI_SLACK_ID noting the
  branch is still pending and will be checked again next cycle (or on manual trigger).
- PR-creation API call fails after confirmation: DM TONI_SLACK_ID with the branch name,
  confirmation status, and the error — the confirmed content is safe on the branch even
  though the PR didn't open.
- Push fails: log the error, DM TONI_SLACK_ID with the branch name and error.
- Any uncaught fatal error: DM TONI_SLACK_ID with the routine name, the failed step,
  and the error text before exiting non-zero.
